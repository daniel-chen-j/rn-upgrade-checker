...
/**
 * Count findings grouped by stable finding code.
 * Helper-only: not included in default report JSON shape.
 * @param {Array<{code?: string}>|null|undefined} findings
 * @returns {Record<string, number>}
 */
function countFindingsByCode(findings) {
  const counts = {};
  for (const finding of findings || []) {
    const code = finding && finding.code ? finding.code : "unknown";
...
function formatHuman(report, options = {}) {
  ...
  if (!quiet && report.hints.length > 0) {
    lines.push("Hints:");
...
module.exports = { buildReport, countFindingsByCode, formatHuman, formatJson, formatSarif };
